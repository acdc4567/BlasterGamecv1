...
FServerSideRewindResult ULagCompensationComponent::ConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation) {
	...
	//Enable Collisions For Head First
	...
	if (World) {
		...
		if (ConfirmHitResult.bBlockingHit) {
			...
			return FServerSideRewindResult{ 1,1 };
			...
			if (ConfirmHitResult.bBlockingHit) {
				...
				return FServerSideRewindResult{ 1,0 };
	...
	return FServerSideRewindResult{ 0,0 };
...
void ULagCompensationComponent::ShowFramePackage(const FFramePackage& Package, const FColor& Color) {
	...
	for (auto& BoxInfo : Package.HitBoxInfo) {
		DrawDebugBox(GetWorld(), BoxInfo.Value.Location, BoxInfo.Value.BoxExtent, FQuat(BoxInfo.Value.Rotation), Color, 0,4.f);
...
void ULagCompensationComponent::Server_ShotgunScoreRequest_Implementation(const TArray<ABlasterCharacter*>& HitCharacters, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations, float HitTime) {
...
void ULagCompensationComponent::Server_ProjectileScoreRequest_Implementation(ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime) {
